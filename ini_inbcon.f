...
cmpk aqua for calculating gaussian thermal forcing
...
C--   1. Read topographical fields (orography, land-sea mask)
      ...
      do j = 1,il
        do i = 1,ix
cmpk aqua          phi0(i,j) = grav0*r4inp(i,j)
cmpk aqua just an initial perturbation; set flat again in ini_invars.f
          phi0(i,j)=2.*grav0*abs(sin(dble(i)/dble(ix)*6.28))
cmpk make everywhere flat           phi0(i,j)=2.*grav0
        enddo
      enddo
cmpk BEGIN putting in Gaussian mountain here; also used in specifying thermal forcing,
cmpk see below.
cmpk this is the Himalaya mountains
...
cmpk this is east Africa highlands
      tpi = 2.0*3.1415926536
      dtr = tpi/360.
      ymolon = 35.*dtr
      ymolat = 5.*dtr
...
cmpk END
      ...
      do j = 1,il
        do i = 1,ix
cmpk aqua          fmask(i,j) = r4inp(i,j)
cmpk put a big polar continent in the NH
c         if (j.ge.29 .and. j.lt.44) then       !asia and europe
c           if(i.le.32)then
c             fmask(i,j) = 1.d0
c           elseif(i.gt.65.and.i.lt.78)then     !north america
c             fmask(i,j) = 1.d0
c           else
c             fmask(i,j) = 0.d0
c           endif
c         elseif(j.ge.15 .and. j.le.29 .and. i.lt.13)then !africa
c           fmask(i,j) = 1.d0
c         elseif(j.ge.12 .and. j.le.29 .and. i.gt.75      !south america
c     &          .and. i.lt.84)then
c           fmask(i,j) = 1.d0
c         elseif(j.ge.15 .and. j.le.21 .and. i.ge.32    !australia
c     &           .and. i.lt.42) then
c           fmask(i,j) = 1.d0
c         else
c           fmask(i,j) = 0.d0
c         endif
c         if(i.le.4 .and. j.ge.15 .and. j.le.25)then !add back gulf of guinea
c           fmask(i,j) = 0.d0
c         endif
cmpk aqua everywhere
          fmask(i,j)=0.d0
c
        enddo
      enddo
...
C--   2.2 Annual-mean surface albedo
      ...
      do j = 1,il
        do i = 1,ix
cmpk aqua          alb0(i,j) = 0.01*r4inp(i,j)
          alb0(i,j) = (1-fmask1(i,j))*0.07d0 + fmask1(i,j)*0.3d0
cmpk make eq. afr higher alb         if(j.ge.21 .and. j.le.27 .and. i.lt.13)then ! 10S-10N Africa
c            alb0(i,j) = 0.1d0
c         endif
        enddo
      enddo
...
cmpk aqua this will be corrected below
          enddo
        enddo
      enddo
...
C--   2.4 Snow depth
      ...
      do it = 1,12
        ...
        do j = 1,il
          do i = 1,ix
cmpk aqua            snowd12(i,j,it) = r4inp(i,j)
            snowd12(i,j,it) = 0.d0             
          enddo
        enddo
      enddo
...
c     read vegetation fraction (in %)
      ...
      do j = 1,il
        do i = 1,ix
cmpk aqua          veg(i,j)=max(0.,0.01*veg(i,j))
            veg(i,j) = 0.d0
cmpk change in eq. afr            if(j.ge.21 .and. j.le.27 .and. i.lt.13)then ! 10S-10N Africa
c             veg(i,j) = 0.9d0 
            endif
        enddo
      enddo
      ...
      do it = 1,12
        ...
        do j = 1,il
          do i = 1,ix
cmpk aqua            swroot = idep2*swl2(i,j)
cmpk aqua            soilw12(i,j,it) = min(1.,rsw*(swl1(i,j)+
cmpk aqua     &                        veg(i,j)*max(0.,swroot-swwil2)))		
            soilw12(i,j,it) = 0.d0
c mpk change in eq. afr            if(j.ge.21 .and. j.le.27 .and. i.lt.13)then ! 10S-10N Africa
c              swroot = idep2*18.d0
c              soilw12(i,j,it) = min(1.,rsw*(15.d0+
c     &                        veg(i,j)*max(0.,swroot-swwil2)))
c            endif
          enddo
        enddo
...
cmpk aqua the following is to make sure it is always running the surface 
cmpk aqua climate calculated above 
         
         do it = 1, 12 
          do j = 1, il/2
           do i = 1, ix
cmpk aqua this should not be necessary since fmask is set to sea everywhere. 
cmpk aqua but just in case...
...
cmpk temperature corrected with topography
        do it = 1,12
         do j = 1, il
          do i = 1, ix
...
cmpk aqua too severe?             oice12(i,j,it) = 1.d0
              sice12(i,j,it) = 0.5d0
             else
              sice12(i,j,it) = 0.d0
             endif    
